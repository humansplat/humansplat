...
def main(args):
    ...
    for i, batch in enumerate(tqdm(testdata, dynamic_ncols=True)):
        ...
        name = batch["name"]
        data = {"body": batch}
        param_dict = pixie.encode(
            data, threthold=True, keep_local=True, copy_and_paste=False
        )
        ...
        if (
            ...
            or args.savePred
            or args.saveImages
            or args.deca_path is not None
        ):
            os.makedirs(os.path.join(savefolder, name), exist_ok=True)
        # -- save results
        # run deca if deca is available and moderator thinks information from face crops is reliable
        if (
            args.deca_path is not None
            and param_dict["moderator_weight"]["head"][0, 1].item() > 0.6
        ):
            cropped_face_savepath = os.path.join(
                savefolder, name, f"{name}_facecrop.jpg"
            )
            cv2.imwrite(
                cropped_face_savepath, util.tensor2image(data["body"]["head_image"][0])
            )
            _, deca_opdict, _ = deca.run(cropped_face_savepath)
            flame_displacement_map = deca_opdict["displacement_map"]
            opdict["displacement_map"] = visualizer.tex_flame2smplx(
                flame_displacement_map
            )
        if args.lightTex:
        ...
        if args.saveVis:
            ...
            if args.showParts:
                ...
                visdict["left_hand"] = data["body"][
                    "left_hand_image"
                ]  # should be flipped
                visdict["right_hand"] = data["body"]["right_hand_image"]
            cv2.imwrite(
                os.path.join(savefolder, f"{name}_vis.jpg"),
                visualizer.visualize_grid(visdict, size=args.render_size),
            )

        if args.saveGif:
            visualizer.rotate_results(
                opdict,
                visdict=visdict,
                savepath=os.path.join(savefolder, f"{name}_vis.gif"),
            )
        if args.saveObj:
        ...
        if args.saveParam:
            ...
            np.savetxt(
                os.path.join(savefolder, name, f"{name}_bbox.txt"),
                batch["bbox"].squeeze(),
            )
        if args.savePred:
            util.save_pkl(
                os.path.join(savefolder, name, f"{name}_prediction.pkl"), opdict
            )
        if args.saveImages:
            for vis_name in visdict.keys():
                cv2.imwrite(
                    os.path.join(savefolder, name, f"{name}_{vis_name}.jpg"),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", default="cuda:0", type=str, help="set device, cpu for using cpu"
    )
    # process test images
    parser.add_argument(
        "--iscrop",
    ...
    # rendering option
    parser.add_argument(
        "--render_size", default=1024, type=int, help="image size of renderings"
    )
    parser.add_argument(
        "--rasterizer_type",
        default="standard",
