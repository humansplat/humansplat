...
from src.models import HumanSplat, get_lr_scheduler, get_optimizer
...
def main():
    ...
    parser.add_argument(
        "--wandb_token_path",
        type=str,
        default="PATH_TO_YOUR_WANDB_TOKEN_FILE",
    ...
    model = HumanSplat(opt, weight_dtype)
    params_to_optimize = filter(lambda p: p.requires_grad, model.parameters())

    # TODO: make it condifurable
    optimizer = get_optimizer(
        params=[
            {
                "params": filter(lambda p: p.requires_grad, model.sv3d.parameters()),
                "lr": configs["optimizer"]["lr"],
            },
            {
                "params": filter(
                    lambda p: p.requires_grad, model.human_rec.parameters()
                ),
                "lr": configs["optimizer"]["lr"],
            },
        ],
    ...
    if args.resume_from_iter is not None:
        ...
        try:
            ...
            logger.info(
                "Load checkpoint by `accelerator.load_state` failed; try only load model parameters"
            )
            safetensors_load_model(
                model.module,
                os.path.join(
                    ckpt_dir, f"{args.resume_from_iter:06d}", "model.safetensors"
                ),
                strict=False,
            )
        if args.use_ema:
    ...
    while True:
        ...
        if args.resume_from_iter is not None and not is_tar_dataset:
            active_train_loader = accelerator.skip_first_batches(
                train_loader,
                num_batches=(
                    global_update_step
                    * args.gradient_accumulation_steps
        ...
        else:  # skip for `TarDataset` is time-consuming
        ...
        for batch in active_train_loader:
            ...
            # Whether visualize denoised images by diffusion models; same freq as evaluation
            vis = (
                (
                    (global_update_step + 1) % configs["train"]["early_eval_freq"] == 0
                    and (  # eval more frequently at the beginning
                        global_update_step + 1
                    )
                    < configs["train"]["eval_freq"]
                )
                or (global_update_step + 1) % configs["train"]["eval_freq"] == 0
                or (global_update_step + 1) == 1
                or (global_update_step + 1) % updated_steps_per_epoch == 0
            )

            with accelerator.accumulate(model):
                outputs = model(batch, weight_dtype, vis=vis)
            ...
            if accelerator.sync_gradients:
                ...
                recon_loss = accelerator.gather(recon_loss.detach()).mean()
                        ...
                        wandb.log(
                            {
                                ...
                                "training/recon_loss": recon_loss.item(),
                ...
                if (
                    ...
                    with torch.no_grad():
                        ...
                        for val_batch in val_loader:
                            val_outputs = model(
                                val_batch, weight_dtype, func_name="evaluate"
                            )
