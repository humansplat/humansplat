...
class HumanSplat(nn.Module):
    def __init__(self, opt: Options, weight_dtype=torch.float16):
        super().__init__()
        self.opt = opt
        self.sv3d = SV3D(opt, weight_dtype)
        self.human_rec = HumanRec(opt, weight_dtype, use_t_cond=True)

        delattr(self.human_rec, "vae")  # not used; to save memory
    ...
    def compute_loss(
        ...
        images = (
            data["images_output"].to(dtype=weight_dtype).to(device)
        )  # (B, V, 3, H, W); [0, 1]
        cam_poses = (
            data["cam_poses_output"].to(dtype=weight_dtype).to(device)
        )  # (B, V, 4)
        ...
        # sigmas = rand_log_normal(shape=(B,), loc=0.7, scale=1.6).to(device)[:, None, None, None, None]  # TODO: P_mean=0.7, P_std=1.6
        sigmas = rand_log_normal(shape=(B,), loc=0.03, scale=0.2).to(device)[
            :, None, None, None, None
        ...
        # Latent Reconstuction model(LRM)
        cam_poses_4x4 = (
            data["cam_poses_4x4_output"].to(dtype=weight_dtype).to(device)
        )  # (B, V, 4, 4)
        ...
        # Use gt conditional images for Latent Reconstuction model(LRM)
        latents = torch.cat(
            [
                gt_latents[:, 0:1, ...],
        ...
        render_outputs, _ = self.human_rec.reconstruct(
            latents,
            cam_poses_4x4,
            ...
            input_data=data if self.opt.load_smpl else None,
        )
        ...
        if self.opt.lambda_lpips > 0.0:
            lpips = self.human_rec.lpips_loss(
        ...
        # recon_batch_loss_weights = torch.clamp(loss_weights.squeeze(), 1., 10.).unsqueeze(1)  # (B, 1); TODO: make it configurable
        outputs["recon_loss"] = recon_loss = recon_loss.mean()
        outputs["loss"] = recon_loss  # TODO: weighting
        return outputs
    ...
    def evaluate(self, data: Dict[str, Tensor], weight_dtype=torch.bfloat16):
        ...
        images = (
            ...
            .to(self.sv3d.image_encoder.device)
        )  # (B, V, 3, H, W); [0, 1]
        cam_poses = (
            data["cam_poses_output"]
            .to(dtype=weight_dtype)
            .to(self.sv3d.image_encoder.device)
        )  # (B, V, 4)
        ...
        for i, t in tqdm(
            ...
            pred_original_latents_lgm = pred_original_latents_lgm[:, random_idxs, ...]
            ...
            latents = noise_scheduler_clone.step(pred_latents, t, latents).prev_sample
        ...
        outputs["lpips"] = self.human_rec.lpips_loss(
            # Downsampled to at most 256 to reduce memory cost
            F.interpolate(
                einops.rearrange(gt_images, "b v c h w -> (b v) c h w") * 2.0 - 1.0,
                (256, 256),
