...
def create_unet_diffusers_config(original_config, image_size: int, controlnet=False):
    ...
    vae_params = (
        original_config.model.params.first_stage_config.params.decoder_config.params
    )
...
def assign_to_checkpoint(
    ...
    for path in paths:
        ...
        new_path = new_path.replace(
            "middle_block.0", f"mid_block.resnets.0{mid_block_suffix}"
        )
        new_path = new_path.replace("middle_block.1", "mid_block.attentions.0")
        new_path = new_path.replace(
            "middle_block.2", f"mid_block.resnets.1{mid_block_suffix}"
        )
...
def convert_ldm_unet_checkpoint(
    ...
    resnet_0_spatial = [
        key for key in resnet_0 if "time_stack" not in key and "time_mixer" not in key
    ]
    ...
    assign_to_checkpoint(
        ...
        unet_state_dict,
        config=config,
        mid_block_suffix="spatial_res_block",
    )

    resnet_0_temporal = [
        key for key in resnet_0 if "time_stack" in key and "time_mixer" not in key
    ]
    ...
    assign_to_checkpoint(
        ...
        unet_state_dict,
        config=config,
        mid_block_suffix="temporal_res_block",
    )

    resnet_1_spatial = [
        key for key in resnet_1 if "time_stack" not in key and "time_mixer" not in key
    ]
    ...
    resnet_1_temporal = [
        key for key in resnet_1 if "time_stack" in key and "time_mixer" not in key
    ]
    ...
    for i in range(num_output_blocks):
        ...
        if len(output_block_list) > 1:
            spatial_resnets = [
                ...
                and (
                    f"output_blocks.{i}.0.time_stack" not in key
                    and "time_mixer" not in key
                )
            ]
            # import ipdb; ipdb.set_trace()
            ...
            temporal_resnets = [
                key
                for key in output_blocks[i]
                if f"output_blocks.{i}.0.time_stack" in key
            ]
            ...
            attentions = [
                key
                for key in output_blocks[i]
                if f"output_blocks.{i}.1" in key and "conv" not in key
            ]
            if len(attentions):
                paths = renew_attention_paths(attentions)
                # import ipdb; ipdb.set_trace()
            ...
            spatial_layers = [
                layer
                for layer in output_block_layers
                if "time_stack" not in layer and "time_mixer" not in layer
            ]
            resnet_0_paths = renew_resnet_paths(
                spatial_layers, n_shave_prefix_segments=1
            )
            # import ipdb; ipdb.set_trace()
            ...
            temporal_layers = [
                layer
                for layer in output_block_layers
                if "time_stack" in layer and "time_mixer" not in key
            ]
            resnet_0_paths = renew_resnet_paths(
                temporal_layers, n_shave_prefix_segments=1
            )
            # import ipdb; ipdb.set_trace()
            for path in resnet_0_paths:
                old_path = ".".join(["output_blocks", str(i), path["old"]])
            ...
            new_checkpoint[
                "up_blocks.0.resnets.0.time_mixer.mix_factor"
            ] = unet_state_dict[f"output_blocks.{str(i)}.0.time_mixer.mix_factor"]
...
def renew_vae_resnet_paths(old_list, n_shave_prefix_segments=0, is_temporal=False):
    ...
    for old_item in old_list:
        ...
        new_item = new_item.replace(
            "mix_factor", "spatial_res_block.time_mixer.mix_factor"
        )
...
def convert_ldm_vae_checkpoint(checkpoint, config):
    ...
    new_checkpoint["decoder.time_conv_out.weight"] = vae_state_dict[
        "decoder.time_mix_conv.weight"
    ]
    new_checkpoint["decoder.time_conv_out.bias"] = vae_state_dict[
        "decoder.time_mix_conv.bias"
    ]
