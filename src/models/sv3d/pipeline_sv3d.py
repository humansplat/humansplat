...
@dataclass
class SV3DPipelineOutput(BaseOutput):
    r"""
    Output class for SV3D pipeline.
...
class SV3DPipeline(DiffusionPipeline):
    r"""
    Pipeline to generate video from an input image using SV3D.
    ...
    def __call__(
        ...
        polars_rad: Optional[List] = None,
        azimuths_rad: Optional[List] = None,
        ...
        if azimuths_rad and polars_rad:
            ...
            added_time_ids = [x.to(device) for x in added_time_ids]
        ...
        # 8. Prepare guidance scale (triangular scaling)
        guidance_scale = torch.linspace(
            min_guidance_scale, max_guidance_scale, num_frames
        ).unsqueeze(0)

        # guidance_scale = torch.cat([
        #     torch.linspace(min_guidance_scale, max_guidance_scale, num_frames//2).unsqueeze(0),
        #     torch.linspace(max_guidance_scale, min_guidance_scale, num_frames - num_frames//2).unsqueeze(0)
        # ], dim=-1)
        ...
        if default_cam is not None:
            B, V, C, H, W = image_latents.shape
            default_cam = F.interpolate(
                default_cam.clone().to(torch.float32), size=(H, W)
            ).to(image_latents.dtype)
            default_cam = default_cam.unsqueeze(0).repeat(B, 1, 1, 1, 1)
        else:
            default_cam = torch.empty(0).to(image_latents.device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat(
                    [latent_model_input, image_latents, default_cam], dim=2
                )
                ...
                noise_pred = self.unet(
                    ...
                    if isinstance(added_time_ids, List)
                    else None,
                    return_dict=False,
                )[0]
        ...
        return SV3DPipelineOutput(frames=frames)


# resizing utils
# TODO: clean up later
def _resize_with_antialiasing(
    input, size, interpolation="bilinear", align_corners=True
):
    h, w = input.shape[-2:]
...
def _gaussian(window_size: int, sigma):
    ...
    if window_size % 2 == 0:
        x = x + 0.5
    gauss = torch.exp(-x.to(torch.float32).pow(2.0) / (2 * sigma.pow(2.0)))
