...
class Visualizer(object):
    ...
    def __init__(
        ...
        self.smplx_texture = imread(self.cfg.model.smplx_tex_path) / 255.0
        self.cached_data = np.load(
            self.cfg.model.flame2smplx_cached_path, allow_pickle=True, encoding="latin1"
        ).item()
        ...
        if background is not None:
            ...
            self.bg = torch.ones(
                [1, 1, self.render_size, self.render_size],
                device=self.device,
                dtype=torch.float32,
            )
    ...
    def _setup_renderer(self):
        ...
        mask = imread(self.cfg.model.face_eye_mask_path).astype(np.float32) / 255.0
        mask = torch.from_numpy(mask[:, :, 0])[None, None, :, :].contiguous()
        self.flame_face_eye_mask = F.interpolate(
            mask, [self.cfg.model.uv_size, self.cfg.model.uv_size]
        ).to(self.device)
        self.smplx_face_eye_mask = self.tex_flame2smplx(
            self.flame_face_eye_mask, np.zeros([uv_size, uv_size, 1], dtype=np.float32)
        )

        # face region mask in flame texture map
        mask = imread(self.cfg.model.face_mask_path).astype(np.float32) / 255.0
        mask = torch.from_numpy(mask[:, :, 0])[None, None, :, :].contiguous()
        self.flame_face_mask = F.interpolate(
            mask, [self.cfg.model.uv_size, self.cfg.model.uv_size]
        ).to(self.device)
        self.smplx_face_mask = self.tex_flame2smplx(
            self.flame_face_mask, np.zeros([uv_size, uv_size, 1], dtype=np.float32)
        )
        ...
        self.color_dict = {
            "head": [180 / 255.0, 0, 0],
            "left_hand": [0, 180 / 255.0, 0],
            "right_hand": [0, 0, 180 / 255.0],
        }
    ...
    def displacement2normal(self, uv_z, coarse_verts, coarse_normals=None):
        ...
        if coarse_normals is None:
            coarse_normals = util.vertex_normals(
                coarse_verts, self.render.faces.expand(coarse_verts.shape[0], -1, -1)
            )
        ...
        uv_detail_normals = (
            uv_detail_normals * self.smplx_face_mask
            + uv_coarse_normals * (1 - self.smplx_face_mask)
        )
    ...
    def tex_flame2smplx(self, flame_texture, smplx_texture=None):
        ...
        if torch.is_tensor(flame_texture):
            ...
            smplx_texture = texture_flame2smplx(
                self.cached_data, flame_texture, smplx_texture
            )
            smplx_texture = (
                torch.from_numpy(smplx_texture.transpose(2, 0, 1))
                .to(device)
                .to(dtype)
                .unsqueeze(0)
            )
            ...
            smplx_texture = texture_flame2smplx(
                self.cached_data, flame_texture, self.smplx_texture
            )
    ...
    def light_albedo(self, opdict):
        ...
        coarse_normals = util.vertex_normals(
            opdict["vertices"],
            self.render.faces.expand(opdict["vertices"].shape[0], -1, -1),
        )
        uv_coarse_normals = self.render.world2uv(coarse_normals).detach()
    ...
    def extract_texture(self, opdict, images):
        ...
        uv_gt = F.grid_sample(
            images, uv_pverts.permute(0, 2, 3, 1)[:, :, :, :2], mode="bilinear"
        )
        ## TODO: poisson blending should give better-looking results
        # extract face only
        uv_texture_gt = uv_gt[:, :3, :, :] * self.smplx_face_eye_mask + (
            uv_texture[:, :3, :, :] * (1 - self.smplx_face_eye_mask)
        )
    ...
    def render_results(
        ...
        shape_images, normal_images, grid = self.render.render_shape(
            opdict["vertices"],
        ...
        visdict = {"inputs": input_images, "shape_images": shape_images}
        ...
        if "displacement_map" in opdict.keys():
            uv_detail_normals = self.displacement2normal(
                opdict["displacement_map"], opdict["vertices"]
            )
            face_detail_normal_images = F.grid_sample(
                uv_detail_normals, grid, align_corners=False
            )
            face_mask = F.grid_sample(self.smplx_face_mask, grid, align_corners=False)
            normal_images = face_detail_normal_images * face_mask + normal_images * (
                1 - face_mask
            )
            shape_detail_images = self.render.render_shape(
                opdict["vertices"],
                opdict["transformed_vertices"],
                detail_normal_images=normal_images,
                background=bg,
                h=h,
                w=w,
            )
        ...
        if moderator_weight is not None:
            ...
            colors = (
                torch.tensor([1.0, 1.0, 0.9])[None, None, :]
                .repeat(batch_size, nv, 1)
                .float()
                .to(self.device)
            )
            for part in moderator_weight.keys():
                ...
                curr_color = (
                    torch.from_numpy(color_map_color(weight)).float().to(self.device)
                )
                colors[:, self.part_idx[part], :] = curr_color[:, None, :]
            opdict["vertex_colors"] = colors
            face_colors = util.face_vertices(
                colors, self.render.faces.expand(batch_size, -1, -1)
            )
            color_shape_images = self.render.render_shape(
                opdict["vertices"],
                opdict["transformed_vertices"],
                background=bg,
                colors=face_colors,
    ...
    def rotate_results(
        ...
        for yaw_angle in range(0, 361, 30):
            ...
            global_pose = rotation_converter.batch_euler2matrix(
                rotation_converter.deg2rad(euler_pose)
            )
            ...
            if result_type == "color":
                ...
                shape_images = self.render.render_shape(
                    vertices, transformed_vertices, background=self.bg
                )
